...
export default function SwipeableCanvas({
  ...
  pages: any[]; currentPageIndex: number;
  onUpdateElement: (id: string, kind: 'photo'|'text'|'sticker', u: any) => void;
  ...
  useEffect(() => { resizeCanvas(); }, [currentPageIndex, drawingMode]);
  useEffect(() => {
  resizeCanvas();
  ...
  const onPointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const ctx = cv.getContext("2d")!;
    ctx.lineJoin = "round"; ctx.lineCap = "round"; ctx.lineWidth = 4; ctx.strokeStyle = "#333";
  ...
  return (
     <div
      ...
      {(page.photos ?? []).map((p: any) => (
        <DraggableElement key={p.id} type="photo" id={p.id}
      ...
      {(page.texts ?? []).map((t: any) => (
        <DraggableElement key={t.id} type="text" id={t.id}
          content={t.text} fontSize={t.fontSize} color={t.color}
      ...
      <div
        ...
        style={{ pointerEvents: drawingMode ? "auto" : "none", opacity: drawingMode ? 1 : 0, transition: "opacity .15s" }}
      >
        <canvas
          ref={canvasRef}
